...
class ErrorBoundary extends Component {
    ...
    render() {
        if (this.state.hasError) {
            ...
            // refers to the component that we are rendering
        }
    }
}
...
/*
    ...
    Method 1 - static getDerivedStateFromError(error)
        This method takes in error and can be used to return the value of a state variable based on the error. And based on this state variable, we can conditionally render our UI.
    ...
    Method 2 - componentDidCatch(error, info)
        This method takes 2 arguments error and info, we cannot use this method to change state & render UI based on state variable but we can console log the errors (displaying error logs is redundant as it is always displayed in development mode).
